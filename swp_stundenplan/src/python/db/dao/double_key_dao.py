...
class DoubleKeyDAO[T](ABC):
    ...
    `DoubleKeyDAO` is supposed to manage.
    """

    __instance: Optional[Self] = None
    """Single instance at runtime of the subclass inheriting from this `DoubleKeyDAO`."""

    __is_initialized: bool = False
    """Whether the single instance is initialized or not."""

    def __new__(cls) -> Self:
        """Constructs a new `DoubleKeyDAO` instance if none exists yet.

        Returns:
            A new `DoubleKeyDAO` instance if none exists yet, otherwise the existing one.
        """
    ...
    def __init__(
        ...
        """Initializes a `DoubleKeyDAO` with all necessary information.
        ...
        subclass inheriting from this `DoubleKeyDAO`. All calls to this method afterwards are
    ...
    def select(self, first_id: int, second_id: int) -> T:
        """Selects an entity with two given ids from the database table.
        ...
        """
        self.__logger.debug(
            f"Selecting {self.__table_name} with ids {first_id}, {second_id}"
        )
        try:
            with Database().create_connection() as connection:
                row: Optional[Row] = connection.execute(
                    self.__select_query, {"first_id": first_id, "second_id": second_id}
                ).fetchone()
            if row:
                entity: T = self._parse_row(row)
                self.__logger.debug(
                    f"Selected {self.__table_name} with ids {first_id}, {second_id}: {entity}"
                )
                return entity
            raise OperationalError(
                f"Failed to select {self.__table_name} with ids {first_id}, {second_id}"
            )
        except Error as error:
            self.__logger.exception(error)
            raise
    ...
    def delete(self, first_id: int, second_id: int) -> None:
        """Deletes an entity with two given ids from the database table.
        ...
        """
        self.__logger.debug(
            f"Deleting {self.__table_name} with ids {first_id}, {second_id}"
        )
        try:
            with Database().create_connection() as connection:
                deleted: bool = (
            ...
            if deleted:
                self.__logger.debug(
                    f"Deleted {self.__table_name} with ids {first_id}, {second_id}"
                )
            else:
                raise OperationalError(
                    f"Failed to delete {self.__table_name} with ids {first_id}, {second_id}, "
