...
class SingleKeyDAO[T](ABC):
    """Represents a Data Access Object (DAO) that abstracts away database tables with a single
    primary key.
    ...
    `SingleKeyDAO` is supposed to manage.
    """

    __instance: Optional[Self] = None
    """Single instance at runtime of the subclass inheriting from this `SingleKeyDAO`."""

    __is_initialized: bool = False
    """Whether the single instance is initialized or not."""

    def __new__(cls) -> Self:
        """Constructs a new `SingleKeyDAO` instance if none exists yet.

        Returns:
            A new `SingleKeyDAO` instance if none exists yet, otherwise the existing one.
        """
    ...
    def __init__(
        ...
        """Initializes a `SingleKeyDAO` with all necessary information.
        ...
        subclass inheriting from this `SingleKeyDAO`. All calls to this method afterwards are
    ...
    def insert(self, entity: T) -> int:
        ...
        try:
            with Database().create_connection() as connection:
                id: Optional[int] = connection.execute(
                    self.__insert_query, entity.__dict__
                ).lastrowid
            if id:
                self.__logger.debug(f"Inserted {self.__table_name} with id {id}")
                return id
            raise OperationalError(f"Failed to insert {self.__table_name}: {entity}")
        except Error as error:
            self.__logger.exception(error)
            raise
    ...
    def select(self, id: int) -> T:
        """Selects an entity with a given id from the database table.

        Args:
            id: Id of the entity to select.
        ...
        """
        self.__logger.debug(f"Selecting {self.__table_name} with id {id}")
        try:
            with Database().create_connection() as connection:
                row: Optional[Row] = connection.execute(
                    self.__select_query, {"id": id}
                ).fetchone()
            if row:
                entity: T = self._parse_row(row)
                self.__logger.debug(
                    f"Selected {self.__table_name} with id {id}: {entity}"
                )
                return entity
            raise OperationalError(f"Failed to select {self.__table_name} with id {id}")
        except Error as error:
            self.__logger.exception(error)
            raise
    ...
    def delete(self, id: int) -> None:
        """Deletes an entity with a given id from the database table.
        ...
        """
        self.__logger.debug(f"Deleting {self.__table_name} with id {id}")
        try:
            with Database().create_connection() as connection:
                deleted: bool = (
                    connection.execute(self.__delete_query, {"id": id}).rowcount
                ) > 0
            if deleted:
                self.__logger.debug(f"Deleted {self.__table_name} with id {id}")
            else:
                raise OperationalError(
                    f"Failed to delete {self.__table_name} with id {id}, no such entity exists"
                )
        except Error as error:
            self.__logger.exception(error)
            raise
